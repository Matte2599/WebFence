...
- Packaging Debian/Windows in implementazione: [ADR-007](DOCS/it/ADR-007-PACKAGING.md). `.deb` nativo Debian 12 con dipendenze ELF derivate, runtime separato senza toolchain e purge; ZIP UCRT64 con chiusura import DLL, avvisi e test PATH Windows. Prime prove XCB richiedono window manager e attesa attivazione nel self-test; normale GUI invariata. Prova Debian ARM64 locale superata (installazione, offscreen/XCB/Openbox, purge). CI Debian x86-64 e Windows ancora da eseguire.
