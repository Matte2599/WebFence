...
Build, vet, module verification, race-enabled pure tests and updated Qt self-test passed locally. macOS Qt bundle verified; IT/EN shortcuts and language restoration after reopening tested in the actual GUI. Initial CI for code `d41aba5`: build/vet/self-test passed on all four targets, Windows/Linux completed. Fixed duplicate C++17 flag in macOS packaging, which invalidated the cache and caused a second compilation; macOS CI uses `-O0 -g0` C++ wrappers to limit cold compilation (~17 minutes observed with `-O2 -g`); local build defaults unchanged. New run to be recorded. [Qt status](DOCS/en/QT-DESKTOP.md), [accepted ADR-002](DOCS/en/ADR-002-GUI.md). The historical comparison (`90c7c7b`) passed Linux Qt CI but had assistive tree/action limitations: migration does not resolve those by itself. M0 open; no scanner, project storage, scope/network, CVE, signing, AI or keychain. No supported release. Assess MIQT maturity, Qt LGPL/commercial obligations and Windows 10 maintenance beyond Qt 6.12. No purchase. Commit/push remain authorized; verify success rather than inferring it from remote configuration.
