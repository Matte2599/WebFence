...
- Per completare M1 servono ancora persistenza e migrazioni del progetto, versione/rinnovo della dichiarazione, policy di metodi/percorsi/IP, broker di produzione con controlli per hop e budget, UI IT/EN, discovery e prove redatte. Il collegamento attuale è limitato al laboratorio loopback: non usarlo per traffico verso target reali.
...
- Completing M1 still requires project persistence/migrations, declaration versioning/renewal, method/path/IP policy, a production broker with per-hop checks and budgets, IT/EN UI, discovery and redacted evidence. The current connection is limited to the loopback laboratory: do not use it for real target traffic.
