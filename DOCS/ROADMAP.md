...
[CI `18c513e` conclusa al secondo tentativo](https://github.com/Matte2599/WebFence/actions/runs/35961360935): sette job verdi; il primo macOS 26 falliva solo per timeout del mirror sorgenti, il rilancio ha completato i materiali. La diagnostica AX rilanciata su 26 fallisce ancora sei stadi non bloccanti. [Variante AX Cocoa privata](it/M0-QT-REPLACEMENT.md): preparato il confronto tra bundle normale e plugin che restituisce direttamente le righe sintetiche, con lo stesso client AX nei due runner macOS. Patch senza fuzz, compilazione e firma locali riuscite; il Mac bloccato impedisce un self-test focus locale conclusivo. Attendere la nuova CI prima di attribuire un effetto AX; M0-01 invariato.
...
[CI `18c513e` completed on attempt two](https://github.com/Matte2599/WebFence/actions/runs/35961360935): seven green jobs; the first macOS 26 attempt failed only on a source-mirror timeout and the rerun completed materials. Its AX diagnostic still fails six nonblocking stages. [Private Cocoa AX variant](en/M0-QT-REPLACEMENT.md): a comparison is prepared between the normal bundle and a plugin returning synthetic rows directly, using the same AX client on both macOS runners. The patch applied without fuzz, and local compilation and signing succeeded; the locked Mac prevents a conclusive local focus self-test. The new CI results are needed before attributing any AX effect; M0-01 is unchanged.
