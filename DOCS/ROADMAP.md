...
[Difetto AX dopo i filtri](evidence/macos-ax-table-reset-2026-09-24.md): su una copia privata del bundle, la sequenza 10.000 → 1 → 0 → 10.000 righe lascia `AXRows` con 10.000 riferimenti ma la prima riga non valida (`kAXErrorInvalidUIElement`). Confermato sui runner macOS 15/26; self-test Qt interno e soak non lo rilevano. Tre varianti private del plugin non hanno corretto il difetto. Le prove locali tardive erano limitate dal Mac bloccato, ma la CI conferma il problema: correggere, ricontrollare a schermo sbloccato e provare un lettore reale. Bundle originale e preferenza personale preservati; M0-01 aperto.
...
Firme sorgenti Windows del 2026-09-24: [otto verifiche OpenPGP offline](evidence/windows-signatures-2026-09-24.md) con sette chiavi pubbliche vincolate, inclusi i fingerprint delle sottochiavi GCC e PCRE2. Stessi 21 archivi/316.641.108 byte: 112 input verificati, zero non verificati nel piano corrente; vecchia raccolta rifiutata, 73 test Python e allegato locale completo superati. [CI `2cd9022`, sei job verdi](https://github.com/Matte2599/WebFence/actions/runs/35945996095). Restano fiducia/identità esterna delle chiavi, componenti incorporati, licenze e collaudi desktop; M0-02/06 aperti.
...
[Diagnostica ripetuta e prova `18c513e`](evidence/macos-ax-table-reset-2026-09-24.md): la versione precedente fallisce quattro stadi AX su macOS 15 e sei su 26, con `-25202` su righe/celle. Il modello sorgente stabile più proxy emette rimozioni/inserimenti di righe senza reset a ogni filtro, ma **i due nuovi bundle falliscono ancora quattro e sei stadi**. Go, race, vet, self-test e soak offscreen passano; Windows, Debian x86-64/ARM64 e Linux nativi sono verdi. La cache Go CI distingue le immagini macOS 15/26. M0-01 richiede indagine Cocoa mirata e VoiceOver reale; M0-01/02/06 restano parziali, M0-03/04/05 completi secondo la [matrice](it/M0-VALIDATION.md).

[CI `18c513e` conclusa al secondo tentativo](https://github.com/Matte2599/WebFence/actions/runs/35961360935): sette job verdi; il primo macOS 26 falliva solo per timeout del mirror sorgenti, il rilancio ha completato i materiali. La diagnostica AX rilanciata su 26 fallisce ancora sei stadi non bloccanti. [Confronto privato `2904f5f`](evidence/macos-ax-table-reset-2026-09-24.md): il plugin Cocoa che restituisce direttamente le righe sintetiche compila e supera firma/self-test/soak nei due runner, ma fallisce le stesse quantità di stadi AX del bundle normale (tre su 15, sei su 26). Ipotesi scartata; prova extra rimossa dalla CI ordinaria, M0-01 invariato.

[CI `a8b65f2`, sette job verdi](https://github.com/Matte2599/WebFence/actions/runs/35966364447) dopo la rimozione della prova diretta dalla CI; AX normale resta fallito e non bloccante. [Rivalidazione privata dell'identità Cocoa](evidence/macos-ax-table-reset-2026-09-24.md): nel sorgente Qt 6.11.2 `isEqual`/`hash` degli elementi sintetici omettono gli indici riga/colonna. Una precedente prova locale era limitata dal Mac bloccato; una variante con indici distinti è compilata e firmata localmente, con originali preservati e soak separato superato (quattro cicli/12,006 s), ma self-test focus inconcludente. La CI macOS 26 confronterà il test AX a tre cicli sul bundle e sulla copia; nessuna correzione di produzione adottata, M0-01 aperto.
...
[AX defect after filtering](evidence/macos-ax-table-reset-2026-09-24.md): on a private bundle copy, the 10,000 → 1 → 0 → 10,000 row sequence leaves `AXRows` reporting 10,000 references while the first row is invalid (`kAXErrorInvalidUIElement`). Confirmed on macOS 15/26 runners; the internal Qt self-test and soak miss it. Three private plugin variants did not fix the defect. Later local trials were limited by the locked Mac, but CI confirms the problem: fix it, recheck on an unlocked screen and try a real reader. Original bundle and personal preference preserved; M0-01 open.
...
Windows source signatures on 2026-09-24: [eight offline OpenPGP checks](evidence/windows-signatures-2026-09-24.md) with seven pinned public keys, including GCC and PCRE2 subkey fingerprints. Same 21 archives/316,641,108 bytes: 112 verified inputs, zero unverified in the current plan; older collection rejected, 73 Python tests and full local attachment passed. [CI `2cd9022`, six jobs passed](https://github.com/Matte2599/WebFence/actions/runs/35945996095). External key trust/identity, embedded components, licenses and desktop trials remain open; M0-02/06 are partial.
...
[Repeated diagnostic and `18c513e` trial](evidence/macos-ax-table-reset-2026-09-24.md): the prior version fails four AX stages on macOS 15 and six on 26, with `-25202` for rows/cells. The stable source model plus proxy emits row removal/insertion without resetting on every filter, but **the two updated bundles still fail four and six stages**. Go, race, vet, offscreen self-test and soak pass; Windows, Debian x86-64/ARM64 and native Linux jobs are green. CI Go cache distinguishes macOS 15/26 runner images. M0-01 needs focused Cocoa investigation and a real VoiceOver trial; M0-01/02/06 remain partial and M0-03/04/05 complete in the [matrix](en/M0-VALIDATION.md).

[CI `18c513e` completed on attempt two](https://github.com/Matte2599/WebFence/actions/runs/35961360935): seven green jobs; the first macOS 26 attempt failed only on a source-mirror timeout and the rerun completed materials. Its AX diagnostic still fails six nonblocking stages. [Private `2904f5f` comparison](evidence/macos-ax-table-reset-2026-09-24.md): the Cocoa plugin returning synthetic rows directly builds and passes signing/self-test/soak on both runners, but fails the same number of AX stages as the normal bundle (three on 15, six on 26). Hypothesis rejected; extra trial removed from routine CI, M0-01 unchanged.

[CI `a8b65f2`, seven green jobs](https://github.com/Matte2599/WebFence/actions/runs/35966364447) after removal of the direct-row trial from CI; normal AX remains failed and nonblocking. [Private Cocoa identity recheck](evidence/macos-ax-table-reset-2026-09-24.md): Qt 6.11.2 source omits row/column indexes from synthesized-element `isEqual`/`hash`. An earlier local trial was limited by the locked Mac; a distinct-index variant compiled and was signed locally, with originals preserved and a separate soak passing (four cycles/12.006 s), but focus self-test inconclusive. macOS 26 CI will compare the three-cycle AX test on the bundle and copy; no production fix adopted, M0-01 open.
