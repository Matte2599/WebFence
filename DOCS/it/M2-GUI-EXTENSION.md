...
L'autotest Qt usa una run e due record CVE/NVD sintetici su loopback. Esercita entrambi i modi di identità NVD, attestazioni valide e in conflitto, reset dei campi obsoleti, export bilingue non firmato e firmato, verifica offline, generazione/rotazione/revoca con un portachiavi sintetico isolato, import con impronta errata, corretta e ID breve, e conservazione della chiave selezionata al cambio lingua. `go test -race` sul core, `go test ./...`, `go vet ./...`, test Python e soak restano regressioni generali.
