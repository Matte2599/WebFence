...
| M0-02 | Build e packaging sui target richiesti | Parziale: build native macOS ARM64, Windows Server x86-64, Ubuntu x86-64/ARM64; bundle macOS di sviluppo. | Pacchetti Windows/Linux, dipendenze/notices, avvio fuori toolchain, sistemi minimi e prova Windows 10/11 e Debian; rischi residui espliciti. |
| M0-03 | GUI, SQLite, JWS, portachiavi | Parziale: [Qt scelto](ADR-002-GUI.md), [SQLite/JWS selezionati e provati](ADR-004-STORAGE-SIGNATURE.md). | [Adattatore portachiavi implementato](ADR-005-CREDENTIALS.md), prove native macOS di lettura/scrittura, blocco e assenza superate; prove runtime CI Linux/Windows e sessione Windows indisponibile ancora da completare. Nessun fallback in chiaro. |
| M0-04 | Versioni, scheletro, cataloghi, CI | Completato sul codice `f1d2a84`: [CI quattro target verde](https://github.com/Matte2599/WebFence/actions/runs/35863431196). | Ripetere sul commit conclusivo di M0 e verificare il remoto. |
...
3. Con sola tastiera caricare fixture (Ctrl/Cmd+O), cercare `DEMO10000` (Ctrl/Cmd+F), passare ai risultati (F6), leggere la prova (Ctrl/Cmd+Shift+E), selezionare/copiare testo. La stringa con tag/script resta testo inerte. Provare Tab e Shift+Tab, menu e pulsante copia, senza trappole di focus.
