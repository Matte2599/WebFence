...
Gli SHA-256 dei 22 archivi binari originali sono ora [vincolati ai checksum pubblicati sulle pagine MSYS2](../evidence/windows-binary-checksum-lock-2026-09-24.md). Il registro revisionato è incluso nello ZIP e la prova sull'archivio estratto ne ricontrolla il legame con `native-build.json`. Le versioni nuove richiedono riesame esplicito; il controllo non sostituisce firme PGP, inventario del codice incorporato o revisione legale. Il packaging verifica inoltre che i riferimenti ai testi di licenza nei JSON di attribuzione Qt selezionati rimangano confinati ai protocolli e puntino a file inclusi; il test dello ZIP richiede 27 riferimenti per la versione bloccata. Per il solo sorgente Git `winpthreads`, il [lock VCS](../evidence/windows-vcs-2026-09-24.md) richiede un tar SHA-256 riprodotto da una repository bare isolata; l'allegato conserva il lock e i metadati di prova. Le [otto firme distaccate delle sorgenti](../evidence/windows-signatures-2026-09-24.md) vengono ora verificate offline con chiavi pubbliche vincolate; autenticità delle chiavi e obblighi di licenza restano da esaminare.

Il packaging richiede ora anche le [22 firme distaccate dei pacchetti binari](../evidence/windows-binary-signatures-2026-09-24.md): archivio e firma devono corrispondere ai byte bloccati, la chiave pubblica al fingerprint ufficiale e la verifica OpenPGP deve riuscire offline prima della pubblicazione dello ZIP. Lock, chiave, firme e risultati sono conservati nel pacchetto di sviluppo; l’ispezione dello ZIP ne controlla l’integrità, mentre la verifica crittografica avviene nella build sui pacchetti originali. Fiducia/revoca indipendenti, composizione dei binari e revisione legale restano aperte. [CI `589fa48`, sei job verdi](https://github.com/Matte2599/WebFence/actions/runs/35947837828).
...
Il container runtime Debian parte dall’immagine slim senza Go, gcc o qmake. Installa il `.deb` tramite APT, esegue i self-test come utente non privilegiato, prima offscreen e poi XCB in Xvfb con Openbox. Una prova separata avvia la GUI normale in una sessione privata Xvfb/D-Bus, legge l'albero AT-SPI in italiano e inglese e controlla la sequenza tabella 10.000 → 1 → 0 → 10.000 con la prima cella valida dopo il reset; [procedura ed evidenza](../evidence/debian-atspi-2026-09-24.md).

La prova [Orca headless](../evidence/debian-orca-headless-2026-09-24.md) usa Speech Dispatcher con configurazione privata `dummy`/ALSA `null` e verifica in EN/IT i comandi vocali generati per la riga sintetica. Python, AT-SPI, D-Bus, Orca, Speech Dispatcher e xdotool sono dipendenze del **solo container di test**, non del `.deb`. Seguono purge e verifica della rimozione di eseguibile/launcher e della conservazione di una preferenza utente sintetica. I log e il checksum sono esportati solo se tutte le fasi passano. Questo prova dipendenze dichiarate e comportamento nel container; non prova audio ascoltato, un desktop Debian reale, Wayland, GPU o installazione fisica.
