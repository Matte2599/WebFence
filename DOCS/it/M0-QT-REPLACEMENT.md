...
`--ax-distinct-identity` rivalida una precedente ipotesi locale: il codice Qt 6.11.2 confronta ID e ruolo sintetico di righe/celle, ma omette gli indici in `isEqual` e `hash`. La variante privata include anche indice di riga e colonna in entrambe le operazioni, mantenendo invariato il plugin di produzione. Il patch set si applica senza fuzz all'archivio verificato; compilazione e firma locali sono riuscite. Il self-test della copia locale falliva nei controlli attivazione/focus durante la sessione Mac bloccata; il soak Cocoa separato ha superato quattro cicli/12,006 s. Nella [CI macOS 26 `0a568ff`](https://github.com/Matte2599/WebFence/actions/runs/35967360307), la copia privata supera firma, self-test e soak, ma il client Swift fallisce **sei stadi su nove**, esattamente come il bundle normale. `CFEqual` distingue già prima e ultima riga nel bundle normale: questa misura non dimostra che la comparazione nativa sia la causa. L'ipotesi non risolve M0-01; la prova aggiuntiva è rimossa dalla CI ordinaria e resta solo riproducibile manualmente. Serve indagare la durata dei riferimenti AX e provare VoiceOver reale.
