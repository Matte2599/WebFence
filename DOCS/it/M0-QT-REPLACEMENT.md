...
`--ax-distinct-identity` rivalida una precedente ipotesi locale su un runner non bloccato: il codice Qt 6.11.2 confronta ID e ruolo sintetico di righe/celle, ma omette gli indici in `isEqual` e `hash`. La variante privata include anche indice di riga e colonna in entrambe le operazioni, mantenendo invariato il plugin di produzione. Il patch set si applica senza fuzz all'archivio verificato; compilazione e firma locali sono riuscite. Il self-test della copia locale fallisce ancora solo su attivazione/focus nella sessione Mac bloccata; il soak Cocoa eseguito separatamente supera quattro cicli/12,006 s. L'effetto AX viene misurato con lo stesso client Swift sul runner macOS 26. Build e diagnostica della variante sono non bloccanti; un esito positivo richiederebbe poi conferma su macOS 15 e con VoiceOver reale.
