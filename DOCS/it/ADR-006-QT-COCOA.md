...
Una [nuova prova AX a stato fermo](../evidence/macos-ax-table-reset-2026-09-24.md) ha isolato un limite non escluso dalla prova breve precedente: dopo 10.000 → 1 → 0 → 10.000 righe tramite filtro, l'array `AXRows` torna a 10.000 ma la prima riga restituisce `kAXErrorInvalidUIElement`. Il self-test Qt interno passa. Due varianti private non hanno corretto il difetto; la terza è inconcludente, perché un controllo UI successivo ha rilevato il Mac bloccato senza stabilire da quando. Quella patch è rimasta invariata durante le prove descritte; servono ricontrollo a schermo sbloccato e lettore reale prima di chiudere M0-01.

## Modello della tabella durante i filtri

La prova CI ripetuta mostra che il reset completo del modello, usato a ogni modifica del filtro, coincide con righe/celle Cocoa con riferimenti AX invalidi. La successiva modifica applicativa conserva le 10.000 fixture nel modello sorgente e usa `QSortFilterProxyModel` per le sole righe visibili. Qt emette così inserimenti/rimozioni di righe durante il filtro; il reset del sorgente resta limitato a carica e svuota. L'identità della selezione continua a basarsi sull'ID indipendente dalla lingua. Nel sorgente Qt 6.11.2 esaminato, `QAccessibleTable::modelChange(ModelReset)` elimina tutte le interfacce figlie dalla cache, mentre `RowsRemoved` conserva quelle con indici persistenti validi: questo motivava l'ipotesi. Il self-test verifica prima e ultima cella accessibile e che i filtri emettano cambiamenti di righe senza `ModelReset`. La [diagnostica nativa dopo la modifica](../evidence/macos-ax-table-reset-2026-09-24.md) fallisce ancora su macOS 15 e 26: i soli segnali di righe non correggono il bridge. M0-01 resta aperto e richiede indagine Cocoa mirata e VoiceOver reale.

## Estensione WebFence: riferimenti Cocoa stabili

La successiva indagine sul sorgente Qt 6.11.2 e sui log `qt.accessibility.table` ha mostrato che il bridge ricrea gli elementi nativi di riga a ogni cambio di conteggio. Il precedente `AXRows` restituiva il numero corretto, ma righe o celle già pubblicate diventavano invalide (`−25202`). Trattenere senza limiti gli array storici ha superato un confronto breve, ma avrebbe fatto crescere la memoria a ogni filtro; trattenerne solo quattro non ha corretto il problema.

La patch consolidata conserva gli elementi di riga in un pool limitato alla **massima dimensione della tabella nella sessione** e riusa le colonne quando il loro numero non cambia. Le celle sintetiche rimangono segnaposto nativi stabili: per ogni interrogazione leggono la cella Qt corrente alla stessa posizione, senza riassegnare il proprio ID alla cache volatile delle interfacce Qt. Il titolo della cella sintetica espone di nuovo il nome. Questa è una modifica WebFence al plugin Qt, distinta dalla proposta 772484; deve rimanere nel pacchetto dei sorgenti corrispondenti e richiede riesame a ogni cambio di versione Qt.

Una nuova build locale macOS 26.6.2 è stata firmata e verificata; self-test Cocoa e client AX di 30 cicli passano, inclusi focus e attivazione della prima cella. Una variante privata identica ha superato altri 30 cicli (92 stadi AX); 93 campioni RSS non mostrano crescita sostenuta in quella breve prova. Non è un collaudo prolungato né un test VoiceOver. La [CI `e2cce25`](https://github.com/Matte2599/WebFence/actions/runs/35975333332) supera sette job: test AX bloccante, sostituzione del plugin e raccolta sorgenti riusciti sia su macOS 15 sia su 26. [Registro bilingue](../evidence/macos-ax-table-reset-2026-09-24.md). M0-01 resta parziale.
