...
La prova CI ripetuta conferma che il reset completo del modello, usato a ogni modifica del filtro, può lasciare righe/celle Cocoa con riferimenti AX invalidi. La successiva modifica applicativa conserva le 10.000 fixture nel modello sorgente e usa `QSortFilterProxyModel` per le sole righe visibili. Qt emette così inserimenti/rimozioni di righe durante il filtro; il reset del sorgente resta limitato a carica e svuota. L'identità della selezione continua a basarsi sull'ID indipendente dalla lingua. Nel sorgente Qt 6.11.2 esaminato, `QAccessibleTable::modelChange(ModelReset)` elimina tutte le interfacce figlie dalla cache, mentre `RowsRemoved` conserva quelle con indici persistenti validi: questo motiva la modifica, ma non ne prova l'efficacia sul bridge Cocoa. Il self-test verifica prima e ultima cella accessibile e che i filtri emettano cambiamenti di righe senza `ModelReset`; resta obbligatoria la diagnostica AX nativa e la prova VoiceOver.
