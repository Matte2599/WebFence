...
Una nuova dichiarazione completa può essere validata con `Project.ReviseAuthorization(AuthorizationDraft)`: conserva ID e nome, incrementa la revisione e lascia intatti progetto e `RunScope` precedenti. `Project.RevokeAuthorization()` crea una revisione non avviabile; un nuovo consenso confermato può riattivarla. `Project.Revision()` e `RunScope.Revision()` identificano la versione. Lo [store v3](M1-PROJECT-STORE.md) salva le revisioni e la revoca, rileva modifiche concorrenti e restituisce solo quella corrente per nuove run. Le vecchie run mantengono il loro limite originale; quelle [gestite dallo store](M1-MANAGED-RUNS.md) vengono anche fermate quando la dichiarazione cambia.
