...
L'[esame tecnico degli import Qt Windows](../evidence/windows-qt-system-imports-2026-09-24.md) collega il `PKGBUILD` del sorgente al `.BUILDINFO` dell'archivio binario firmato e legge gli import PE di quattro componenti del pacchetto. Le otto opzioni `FEATURE_system_*` esaminate corrispondono a dipendenze dinamiche osservate; `Qt6Core.dll` importa anche `libzstd.dll`. È evidenza del pacchetto Qt, non prova che ogni plugin sia presente nello ZIP WebFence o che non vi sia codice incorporato. Serve il confronto con l'inventario dello ZIP e la revisione legale.
