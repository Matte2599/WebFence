...
L'input Git `winpthreads` della versione bloccata è ora verificato offline; eventuali altri VCS restano `vcs_unverified`. Firme con `SKIP` e checksum deboli restano `no_strong_checksum`. SHA-1/MD5 vengono eventualmente confrontati, ma non bastano per assegnare `verified`. Gli hash degli archivi esterni sono osservati, non confrontati con firme indipendenti. `distribution_ready` e `corresponding_sources_complete` restano **false**.
...
Il [nuovo registro di prova](../evidence/windows-vcs-2026-09-24.md) documenta il lock versionato `winpthreads`, la verifica Git isolata integrata nel collector e la raccolta locale completa: 21 archivi immutati, **104 input verificati** incluso Git e otto firme con `SKIP` ancora non verificate. L'allegato reesamina il commit e il tar, include il lock e rifiuta la vecchia raccolta con `vcs_unverified`; il test dello ZIP estratto controlla il lock e i metadati di verifica. 68 regressioni Python locali e prova di raccolta/allegato complete superate; CI del codice modificato in attesa. Serve Git disponibile nell'ambiente di raccolta. Non è una verifica PGP né un'approvazione della distribuzione.
