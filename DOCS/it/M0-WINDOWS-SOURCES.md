...
L'[esame tecnico degli import Qt Windows](../evidence/windows-qt-system-imports-2026-09-24.md) collega il `PKGBUILD` del sorgente al `.BUILDINFO` dell'archivio binario firmato e legge gli import PE di quattro componenti del pacchetto. Le otto opzioni `FEATURE_system_*` esaminate corrispondono a dipendenze dinamiche osservate; `Qt6Core.dll` importa anche `libzstd.dll`. Questa prima prova riguarda il pacchetto Qt, non da sola i plugin dello ZIP WebFence o il codice incorporato. Il confronto con lo ZIP segue sotto; la revisione legale resta aperta.

L'[inventario PE dello ZIP](../evidence/windows-pe-import-inventory-2026-09-24.md) registra ora gli import statici dell'eseguibile e delle DLL già vincolate agli hash; il test sullo ZIP estratto rifiuta DLL non inventariate e import senza corrispondenza con una DLL inclusa o un import di sistema registrato. [CI `e4247b3`, sei job verdi](https://github.com/Matte2599/WebFence/actions/runs/35949425830): 43 DLL/44 PE verificati, 15 file Qt identificati; `qjpeg.dll` è nello ZIP, `qsqlite.dll` no. Caricamenti dinamici, componenti incorporati, desktop reali e revisione legale restano fuori da questa prova.
...
Da PowerShell 7, la prova completa è `./scripts/test-windows-package.ps1 -Archive ./dist/webfence-windows-amd64.zip -RequireSources`. I controlli dei sorgenti precedono quelli GUI con PATH di solo sistema. L’assemblaggio conserva `distribution_ready=false` e `corresponding_sources_complete=false`: completezza degli avvisi e dei componenti incorporati, fiducia nelle chiavi, ambiente di build, ricompilazione/sostituzione e termini di distribuzione richiedono ancora revisione.
