...
Aggiornamento: 2026-09-24. Solo artefatti di sviluppo; nessuna release supportata o revisione della distribuzione completata.
...
Debian 12 offre Qt 6.4.2; Debian 13 Qt 6.8.2 al momento della verifica. Il `.deb` nativo ora viene compilato e provato in Debian 12, con dipendenze ELF derivate e Qt fornito dal sistema. Entrambe le architetture amd64/arm64 hanno superato installazione, self-test offscreen/XCB e purge nel runtime container senza toolchain ([CI iniziale](https://github.com/Matte2599/WebFence/actions/runs/35876057107)). Successivamente sono stati aggiunti il [controllo AT-SPI](../evidence/debian-atspi-2026-09-24.md) e la [prova Orca dei comandi vocali EN/IT](../evidence/debian-orca-headless-2026-09-24.md) con uscita audio nulla. Queste prove non equivalgono a un desktop fisico, Wayland o audio ascoltato. [ADR-007](ADR-007-PACKAGING.md) descrive procedura e limiti.
...
Inclusione facoltativa ora implementata con `WEBFENCE_NATIVE_SOURCE_MATERIALS`: [procedura](M0-SOURCE-MATERIALS.md). Rigenera gli avvisi dagli archivi nuovamente verificati e li aggiunge prima della firma; gli archivi completi restano separati. Bundle locale con 247 avvisi, firma e self-test Cocoa verificati. Anche i supplementi GLib/libb2 sono ora integrati facoltativamente prima della firma, con ricette vincolate agli hash; la revisione della distribuzione resta aperta.

## Versione minima dichiarata dall’artefatto

Il packaging ora legge `LC_BUILD_VERSION` o `LC_VERSION_MIN_MACOSX` da ogni Mach-O incluso, dopo il controllo ARM64. Registra il valore per file in `minimum_macos` e il massimo nell’oggetto `macos_minimum` dell’inventario. Distingue il deployment target dalla versione dell’SDK e del linker; piattaforme diverse da macOS, valori assenti/ambigui o versioni non valide fermano il packaging.

`scripts/macho_minos.py` aggiorna `Contents/Info.plist` con `LSMinimumSystemVersion` nel formato numerico `n.n.n`, conservando una dichiarazione preesistente più alta. La scrittura avviene esclusivamente nello staging prima della firma, senza modificare i load command dei binari. Una dichiarazione distinta per architettura richiede revisione esplicita; il builder corrente produce solo ARM64.

[Apple documenta questa chiave](https://developer.apple.com/library/archive/documentation/General/Reference/InfoPlistKeyReference/Articles/LaunchServicesKeys.html#//apple_ref/doc/uid/20001431-113253) come requisito minimo usato da Launch Services per segnalare sistemi insufficienti. Il valore calcolato è un limite conservativo per tutti i binari inclusi, **non una certificazione di compatibilità o una scelta del minimo ufficialmente supportato**. Non vengono abbassati i requisiti delle dipendenze; una build diversa può dichiarare un minimo diverso.

Difetto riprodotto sul bundle precedente: eseguibile e 18 altri Mach-O richiedono macOS 26, ma `LSMinimumSystemVersion` è assente. Cinque nuove regressioni verificano comandi moderni/legacy, confronto numerico, separazione fra SDK e minimo, rifiuti e conservazione di plist non validi; 36 test Python complessivi passati localmente. Nuovo bundle: valore `26.0.0`, 19 Mach-O con minimo 26 e nove con minimo 14. Verifica indipendente con `vtool` sui 28 file, firma ad hoc e self-test Cocoa superati; CI completata come descritto sotto. Non è stata provata la finestra di errore di Launch Services su un macOS inferiore al requisito.

[CI `3593dcc`](https://github.com/Matte2599/WebFence/actions/runs/35914285406): sei job superati, 36 regressioni Python sui quattro target. Bundle del runner macOS 15: minimo dichiarato/Mach-O 15.0.0 e 28 file ricontrollati con vtool, firma e Cocoa superati.

| Artefatto verificato | Minimo nel plist | Mach-O ricontrollati | Prova di esecuzione |
| --- | --- | --- | --- |
