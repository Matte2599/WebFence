# Documentazione / Documentation

**IT:** Specifiche iniziali del 23 settembre 2026. «Previsto», «deve» e i criteri di accettazione descrivono il prodotto da costruire, non funzioni disponibili. I documenti IT/EN devono evolvere nella stessa modifica.

**EN:** Initial specifications dated September 23, 2026. “Planned”, “must” and acceptance criteria describe the product to build, not available features. IT/EN documents must evolve in the same change.

| Documento / Document | Italiano | English |
| --- | --- | --- |
| Presentazione / Overview | [README](../README.md) | [README](../README.en.md) |
| Prodotto e requisiti / Product and requirements | [IT](it/PRODUCT.md) | [EN](en/PRODUCT.md) |
| Architettura / Architecture | [IT](it/ARCHITECTURE.md) | [EN](en/ARCHITECTURE.md) |
| Go e Rust / Go and Rust | [ADR-001 IT](it/ADR-001-LANGUAGE.md) | [ADR-001 EN](en/ADR-001-LANGUAGE.md) |
| Motore di scansione / Scanning engine | [IT](it/SCANNING.md) | [EN](en/SCANNING.md) |
| AI e modelli locali / AI and local models | [IT](it/AI.md) | [EN](en/AI.md) |
| CVE e fonti / CVE and intelligence | [IT](it/VULNERABILITY-INTELLIGENCE.md) | [EN](en/VULNERABILITY-INTELLIGENCE.md) |
| Report e firme / Reports and signatures | [IT](it/REPORTING.md) | [EN](en/REPORTING.md) |
| Dati, memoria e fix / Data, memory and fixes | [IT](it/DATA-AND-RETEST.md) | [EN](en/DATA-AND-RETEST.md) |
| Modello delle minacce / Threat model | [IT](it/THREAT-MODEL.md) | [EN](en/THREAT-MODEL.md) |
| Qualità e benchmark / Quality and benchmarks | [IT](it/QUALITY.md) | [EN](en/QUALITY.md) |
| Sviluppo, installazione e lingue / Development, deployment and languages | [IT](it/DEVELOPMENT.md) | [EN](en/DEVELOPMENT.md) |
| Prototipo desktop M0 / M0 desktop prototype | [IT](it/M0-DESKTOP.md) | [EN](en/M0-DESKTOP.md) |
| Confronto toolkit GUI / GUI toolkit comparison | [IT](it/GUI-COMPARISON.md) | [EN](en/GUI-COMPARISON.md) |
| Decisione GUI / GUI decision | [ADR-002 IT](it/ADR-002-GUI.md) | [ADR-002 EN](en/ADR-002-GUI.md) |
| Interfaccia e usabilità / Interface and usability | [IT](it/UX.md) | [EN](en/UX.md) |
| Dossier revisione legale / Legal review dossier | [IT](it/LEGAL-REVIEW.md) | [EN](en/LEGAL-REVIEW.md) |
| Licenze e governance / Licensing and governance | [IT](it/LICENSING.md) | [EN](en/LICENSING.md) |
| Fonti e limiti della ricerca / Sources and research limits | [IT](it/REFERENCES.md) | [EN](en/REFERENCES.md) |
| Trasporto M0 e confini di rete / M0 transport and network boundaries | [ADR-003 IT](it/ADR-003-TRANSPORT.md) | [ADR-003 EN](en/ADR-003-TRANSPORT.md) |
| Scope origini e laboratorio M0 / Origin scope and M0 lab | [IT](it/M0-SCOPE.md) | [EN](en/M0-SCOPE.md) |
| Verifica e chiusura M0 / M0 verification and closure | [IT](it/M0-VALIDATION.md) | [EN](en/M0-VALIDATION.md) |
| Prerequisiti umani M1 / M1 human prerequisites | [IT](it/M1-PREREQUISITES.md) | [EN](en/M1-PREREQUISITES.md) |
| Verifica e gate M1 / M1 validation and gates | [IT](it/M1-VALIDATION.md) | [EN](en/M1-VALIDATION.md) |
| Progetto e autorizzazione M1 / M1 project and authorization | [IT](it/M1-PROJECT-AUTHORIZATION.md) | [EN](en/M1-PROJECT-AUTHORIZATION.md) |
| Laboratorio autorizzato M1 / M1 authorized lab | [IT](it/M1-AUTHORIZED-LAB.md) | [EN](en/M1-AUTHORIZED-LAB.md) |
| Store progetti M1 / M1 project store | [IT](it/M1-PROJECT-STORE.md) | [EN](en/M1-PROJECT-STORE.md) |
| Run gestite e revoca M1 / M1 managed runs and revocation | [IT](it/M1-MANAGED-RUNS.md) | [EN](en/M1-MANAGED-RUNS.md) |
| Primo controllo HTTP M1 / First M1 HTTP check | [IT](it/M1-HEADER-LAB.md) | [EN](en/M1-HEADER-LAB.md) |
| Discovery HTML M1 / M1 HTML discovery | [IT](it/M1-DISCOVERY-LAB.md) | [EN](en/M1-DISCOVERY-LAB.md) |
| Visite HTTP controllate M1 / M1 controlled HTTP visits | [IT](it/M1-CONTROLLED-CRAWL.md) | [EN](en/M1-CONTROLLED-CRAWL.md) |
| IP pubblici fissati / Pinned public IPs | [ADR-008 IT](it/ADR-008-PINNED-PUBLIC-TRANSPORT.md) | [ADR-008 EN](en/ADR-008-PINNED-PUBLIC-TRANSPORT.md) |
| SQLite e JWS / SQLite and JWS | [ADR-004 IT](it/ADR-004-STORAGE-SIGNATURE.md) | [ADR-004 EN](en/ADR-004-STORAGE-SIGNATURE.md) |
| Pacchetti Debian/Windows / Debian/Windows packages | [ADR-007 IT](it/ADR-007-PACKAGING.md) | [ADR-007 EN](en/ADR-007-PACKAGING.md) |
| Sorgenti Windows / Windows sources | [IT](it/M0-WINDOWS-SOURCES.md) | [EN](en/M0-WINDOWS-SOURCES.md) |
| Materiali sorgente / Source materials | [IT](it/M0-SOURCE-MATERIALS.md) | [EN](en/M0-SOURCE-MATERIALS.md) |
| Ricognizione packaging / Packaging investigation | [IT](it/M0-PACKAGING.md) | [EN](en/M0-PACKAGING.md) |
| Portachiavi nativo / Native credentials | [ADR-005 IT](it/ADR-005-CREDENTIALS.md) | [ADR-005 EN](en/ADR-005-CREDENTIALS.md) |
| Ricompilazione e sostituzione Cocoa / Cocoa rebuild and replacement | [IT](it/M0-QT-REPLACEMENT.md) | [EN](en/M0-QT-REPLACEMENT.md) |
| Correzione Qt Cocoa / Qt Cocoa correction | [ADR-006 IT](it/ADR-006-QT-COCOA.md) | [ADR-006 EN](en/ADR-006-QT-COCOA.md) |
| Desktop Qt corrente / Current Qt desktop | [IT](it/QT-DESKTOP.md) | [EN](en/QT-DESKTOP.md) |

Documenti bilingui condivisi / Shared bilingual documents:

- [ROADMAP.md](ROADMAP.md)
- [LICENSE](../LICENSE)
- [CONTRIBUTING.md](../CONTRIBUTING.md)
- [SECURITY.md](../SECURITY.md)
- [MEMORY.md](../MEMORY.md)
- [AGENTS.md](../AGENTS.md)

- Stabilità prolungata / Prolonged stability: [IT](it/M0-STABILITY.md) · [EN](en/M0-STABILITY.md).
