...
Later blocks add [bounded HTML discovery](M1-DISCOVERY-LAB.md) and [controlled HTTP visits](M1-CONTROLLED-CRAWL.md) in a separate core path; `RunHeaderLab` still visits loopback seeds only. M1 still needs persistent run data and quota/recovery, IT/EN desktop results and visible incomplete coverage. The [human prerequisite plan](M1-PREREQUISITES.md) remains open.
