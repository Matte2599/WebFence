...
Later blocks added [bounded HTML discovery](M1-DISCOVERY-LAB.md) and [controlled HTTP visits](M1-CONTROLLED-CRAWL.md) in a separate core path; `RunHeaderLab` still visits loopback seeds only. Persistence, quota/recovery, IT/EN desktop results and incomplete coverage are in the [current M1 status](M1-VALIDATION.md). The [historical human trial plan](M1-PREREQUISITES.md) remains to be performed for corresponding claims.
