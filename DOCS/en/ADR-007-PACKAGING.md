...
The ZIP records each DLL with copied/original hashes, its owning MSYS2 package, `pacman -Qi` metadata and copies of its notices. Selected notices are hash-compared with their members in cached MSYS2 binary archives and rechecked after ZIP extraction: [43 DLLs, 22 packages and 74 notices in the first CI trial](../evidence/windows-notice-linkage-2026-09-24.md); the [current selection contains 136 files](../evidence/windows-attribution-sidecars-2026-09-24.md). Qt files rewritten by windeployqt may differ from their originals. Dependency licenses are not replaced by WebFence licensing. Complete embedded-component inventory, corresponding sources and distribution materials remain pending. CI builds and tests packages without publishing a release or automatically uploading binary artifacts.

SHA-256 values for the 22 original binary archives are now [bound to checksums published on MSYS2 package pages](../evidence/windows-binary-checksum-lock-2026-09-24.md). The reviewed lock is included in the ZIP, and the extracted-archive trial rechecks its relationship to `native-build.json`. New versions require explicit review; this check does not replace PGP signatures, embedded-code inventory or legal review. Packaging also checks that references to license texts in selected Qt attribution JSON remain within the protocol subtree and point to included files; the ZIP trial requires 27 references for the locked version. For the `winpthreads` Git source alone, the [VCS lock](../evidence/windows-vcs-2026-09-24.md) requires a SHA-256 tar reproduced from an isolated bare repository; the attachment retains the lock and trial metadata. The [eight detached source signatures](../evidence/windows-signatures-2026-09-24.md) are now verified offline against pinned public keys; key authenticity and licensing obligations still need review.

Packaging now also requires the [22 detached binary package signatures](../evidence/windows-binary-signatures-2026-09-24.md): archive and signature must match pinned bytes, the public key must match the official fingerprint and offline OpenPGP verification must pass before publishing the ZIP. Lock, key, signatures and results are retained in the development package; inspection of the ZIP checks their integrity, while cryptographic verification runs during the build against original packages. Independent trust/revocation, binary composition and legal review remain open. [CI `589fa48`, six jobs passed](https://github.com/Matte2599/WebFence/actions/runs/35947837828).
...
The Debian runtime container starts from a slim image without Go, gcc or qmake. It installs the `.deb` through APT and runs self-tests as an unprivileged user, first offscreen and then XCB in Xvfb with Openbox. A separate trial runs the normal GUI in a private Xvfb/D-Bus session, reads the AT-SPI tree in English and Italian and checks the table sequence 10,000 → 1 → 0 → 10,000 with a valid first cell after reset; [procedure and evidence](../evidence/debian-atspi-2026-09-24.md).

The [headless Orca trial](../evidence/debian-orca-headless-2026-09-24.md) uses Speech Dispatcher with private `dummy`/ALSA `null` configuration and checks EN/IT speech commands generated for a synthetic row. Python, AT-SPI, D-Bus, Orca, Speech Dispatcher and xdotool are dependencies of the **test container only**, not the `.deb`. Package purge then checks executable/launcher removal and preservation of a synthetic user preference. Logs and checksums export only if all stages pass. This checks declared dependencies and package behavior in the container; it does not prove heard audio, a real Debian desktop, Wayland, GPU or physical installation.
