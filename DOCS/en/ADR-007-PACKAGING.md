...
Packaging now also requires the [22 detached binary package signatures](../evidence/windows-binary-signatures-2026-09-24.md): archive and signature must match pinned bytes, the public key must match the official fingerprint and offline OpenPGP verification must pass before publishing the ZIP. Lock, key, signatures and results are retained in the development package; inspection of the ZIP checks their integrity, while cryptographic verification runs during the build against original packages. Independent trust/revocation, binary composition and legal review remain open. Changed-code CI pending.
