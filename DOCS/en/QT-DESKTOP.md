...
This document records the historical M0 block at the stated date. For the current M1 scanning desktop see the [M1 matrix](M1-VALIDATION.md) and [guide](DEVELOPMENT.md); “no scanner” statements below describe only that block.
...
An end-user Windows installer, signing/notarization, real Windows 10/11 and Ubuntu 24.04 desktop execution, a macOS machine without development tools and trials on the now-decided [target minimum versions](M1-SUPPORT-POLICY.md) remain open. Windows 10 maintenance after Qt 6.12 is also open. WebFence licensing is unchanged; Qt/MIQT inventory and obligations must be completed before distribution. No purchase or commercial license entered into.
