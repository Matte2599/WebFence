...
`--ax-distinct-identity` rechecks an earlier local hypothesis: Qt 6.11.2 compares the ID and synthesized role of rows/cells but omits their indexes from `isEqual` and `hash`. The private variant includes row and column indexes in both operations while leaving the product plugin unchanged. The patch applies without fuzz to the verified archive; local compilation and signing passed. The local copy's self-test failed activation/focus checks during the locked Mac session; a separate Cocoa soak passed four cycles/12.006 s. In [macOS 26 CI `0a568ff`](https://github.com/Matte2599/WebFence/actions/runs/35967360307), the private copy passes signing, self-test and soak, but the Swift client fails **six of nine stages**, exactly like the normal bundle. `CFEqual` already distinguishes the first and last row in the normal bundle: this observation does not establish native comparison as the cause. The hypothesis does not resolve M0-01; the extra trial is removed from routine CI and remains available for manual reproduction only. AX reference lifetime investigation and a real VoiceOver trial are still needed.
