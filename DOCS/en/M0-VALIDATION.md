...
| M0-04 | Versions, skeleton, catalogs, CI | Complete on code `8fd55b9`: [six passing CI jobs](https://github.com/Matte2599/WebFence/actions/runs/35879464771), including Debian and Windows packages with separate runtimes. | Repeat on the final M0 code commit and verify the remote. |
...
Minimum table metrics are now calculated after `EnsurePolished`, once the native style initializes fonts and sections; the self-test also records actual dimensions. Bundle with AX patch 772484: local Cocoa self-tests at scales 1/1.5/2 passed (viewport 75, two rows 60, table minimum 98 points). Correction on the macOS 15 runner still to confirm in CI.

Follow-up diagnosis: [CI `6d711e5`](https://github.com/Matte2599/WebFence/actions/runs/35884948483) passes five jobs but confirms the Cocoa defect: viewport 59 points for two rows requiring 60, actual header 21 and scrollbar 15. Polish alone does not correct the estimate. The minimum is now recalculated on the header’s `geometriesChanged` signal using actual height and native minimum too. The existing check remains unchanged; added a taller-header regression, failing on the previous code and passing on the new code with the offscreen backend. Correction CI to verify.
