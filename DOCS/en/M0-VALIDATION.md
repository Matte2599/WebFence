...
| M0-03 | GUI, SQLite, JWS, keychain | Partial: [Qt selected](ADR-002-GUI.md), [SQLite/JWS selected and tested](ADR-004-STORAGE-SIGNATURE.md). | [Keychain adapter implemented](ADR-005-CREDENTIALS.md), native macOS round-trip/locked/absent tests passed; Linux/Windows CI runtime checks and Windows unavailable-session trial pending. No plaintext fallback. |
...
3. Using only the keyboard, load fixtures (Ctrl/Cmd+O), search `DEMO10000` (Ctrl/Cmd+F), move to results (F6), read evidence (Ctrl/Cmd+Shift+E), select/copy text. The tag/script string remains inert text. Try Tab and Shift+Tab, menus and the copy button without focus traps.
