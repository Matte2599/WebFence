...
The [22 detached binary signatures](../evidence/windows-binary-signatures-2026-09-24.md) were retrieved from the MSYS2 mirror and verified offline against original archives already bound to the SHA-256 lock. Packaging requires the public key derived from the reviewed MSYS2 keyring package, checks the fingerprint and signature with isolated GnuPG, and retains the lock, key, signatures and per-package evidence in the ZIP. The extracted-ZIP test rechecks bytes and inventory; original binary archives are absent from the ZIP, so cryptographic verification happens during packaging. **77 local Python regressions** and the real 22-archive verification passed; [CI `589fa48`, six jobs passed](https://github.com/Matte2599/WebFence/actions/runs/35947837828). Independent signer identity, key revocation/trust, keyring terms and component licenses still require review.

The [technical Windows Qt import review](../evidence/windows-qt-system-imports-2026-09-24.md) links the source `PKGBUILD` to the signed binary archive's `.BUILDINFO` and reads PE imports from four package components. The eight inspected `FEATURE_system_*` options match observed dynamic dependencies; `Qt6Core.dll` also imports `libzstd.dll`. This first trial concerns the Qt package, not by itself the plugins in the WebFence ZIP or embedded code. The ZIP comparison follows below; legal review remains open.

The [ZIP PE inventory](../evidence/windows-pe-import-inventory-2026-09-24.md) now records static imports of the executable and DLLs already bound to hashes; the extracted-ZIP trial rejects unlisted DLLs and imports without a matching bundled DLL or recorded system import. [CI `e4247b3`, six passing jobs](https://github.com/Matte2599/WebFence/actions/runs/35949425830): 43 DLLs/44 PE files verified, 15 Qt files identified; `qjpeg.dll` is in the ZIP, `qsqlite.dll` is not. Dynamic loads, embedded components, real desktops and legal review remain outside this check.
