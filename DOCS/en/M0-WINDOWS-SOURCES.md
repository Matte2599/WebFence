...
The [technical Windows Qt import review](../evidence/windows-qt-system-imports-2026-09-24.md) links the source `PKGBUILD` to the signed binary archive's `.BUILDINFO` and reads PE imports from four package components. The eight inspected `FEATURE_system_*` options match observed dynamic dependencies; `Qt6Core.dll` also imports `libzstd.dll`. This is evidence about the Qt package, not proof that every plugin is shipped in the WebFence ZIP or that embedded code is absent. Compare it with the ZIP inventory and obtain legal review.
