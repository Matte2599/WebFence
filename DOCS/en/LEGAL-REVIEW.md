...
- [Eight detached Windows source signatures](../evidence/windows-signatures-2026-09-24.md): offline OpenPGP verification with seven fingerprint-pinned public keys; signer identity, key trust and distribution obligations remain open.
- [Twenty-two Windows binary package signatures](../evidence/windows-binary-signatures-2026-09-24.md): public key extracted from an SHA-256-pinned MSYS2 keyring release; review independent identity/trust and obligations for redistributing the key and metadata from the GPL-3.0-or-later package.
- [Windows Qt imports and system-library options](../evidence/windows-qt-system-imports-2026-09-24.md): the recipe linked to `.BUILDINFO` and PE imports indicate dynamic dependencies in examined package components; on their own they do not establish which plugins are shipped in the ZIP or rule out embedded code.
- [Windows ZIP PE inventory](../evidence/windows-pe-import-inventory-2026-09-24.md): 15 Qt files identified in the verified ZIP, including `qjpeg.dll` but not `qsqlite.dll`; static imports recorded for 44 PE files. Embedded code, dynamic loading and obligations for components actually shipped still require review.
