...
Status: M1–M4 logical model; only the [v3 project-metadata and authorization-revision schema](M1-PROJECT-STORE.md), including revocation, is implemented. The other entities, complete retention and retesting remain planned. SQLite driver selection and temporary M0 experiments are in [ADR-004](ADR-004-STORAGE-SIGNATURE.md).
