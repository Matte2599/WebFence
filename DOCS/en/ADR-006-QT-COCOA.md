...
The patch adopted at that point replaced the earlier two guards entirely with production `.h`/`.mm` changes only. Upstream status NEW verified on 2026-09-23; no approval or inclusion in a release presumed. Separate proposal 772485 for cells without a position is not applied, as that case was not reproduced.
...
A [new steady-state AX trial](../evidence/macos-ax-table-reset-2026-09-24.md) isolated a limitation not excluded by the earlier short trial: after filtering 10,000 → 1 → 0 → 10,000 rows, `AXRows` again reports 10,000 but its first row returns `kAXErrorInvalidUIElement`. The internal Qt self-test passes. Two private variants did not fix it; the third is inconclusive because a later UI check found the Mac locked without establishing when. That patch remained unchanged during the trials described; an unlocked-screen retest and real reader trial are needed before closing M0-01.

## Table model while filtering

The repeated CI trial shows that full model resets on filter changes coincide with Cocoa rows/cells having invalid AX references. The subsequent application change keeps all 10,000 fixtures in the source model and uses `QSortFilterProxyModel` for visible rows. Qt then emits row insertion/removal during filtering; source resets are restricted to load and clear. Selection identity remains based on the language-independent ID. In the inspected Qt 6.11.2 source, `QAccessibleTable::modelChange(ModelReset)` deletes all child interfaces from its cache, while `RowsRemoved` retains those with valid persistent indexes. This motivated the hypothesis. The self-test checks the first and last accessible cells and that filtering emits row changes without `ModelReset`. The [native diagnostic after the change](../evidence/macos-ax-table-reset-2026-09-24.md) still fails on macOS 15 and 26: row signals alone do not fix the bridge. M0-01 remains open and needs focused Cocoa investigation and a real VoiceOver trial.

## WebFence extension: stable Cocoa references

Subsequent investigation of Qt 6.11.2 source and `qt.accessibility.table` logs showed that the bridge recreates native row elements on each count change. `AXRows` reported the correct count while previously published rows or cells became invalid (`−25202`). Retaining every old array passed a short comparison but would grow memory with each filter; retaining only four old arrays did not fix the problem.

The consolidated patch keeps row elements in a pool bounded by the **largest table size in the session** and reuses columns while their count is unchanged. Synthetic cells remain stable native placeholders: each query resolves the current Qt cell at the same position without reassigning the native ID to a volatile Qt interface cache entry. The synthetic cell title exposes the name again. This is a WebFence change to the Qt plugin, distinct from proposal 772484; it must remain in the corresponding-source package and be reviewed for each Qt version change.

A newly built local macOS 26.6.2 bundle was signed and verified; the Cocoa self-test and a 30-round AX client passed, including focus and press on the first cell. An identical private variant passed another 30 rounds (92 AX stages); 93 RSS samples showed no sustained growth in that short trial. This is neither a prolonged test nor a VoiceOver trial. [CI `e2cce25`](https://github.com/Matte2599/WebFence/actions/runs/35975333332) passes all seven jobs: the blocking AX test, plugin replacement and source collection succeeded on both macOS 15 and 26. [Bilingual ledger](../evidence/macos-ax-table-reset-2026-09-24.md). M0-01 remains partial.
