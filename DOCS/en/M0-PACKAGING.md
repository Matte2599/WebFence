...
Debian 12 provides Qt 6.4.2; Debian 13 provides Qt 6.8.2 at inspection time. The native `.deb` is now built and tested in Debian 12, with derived ELF dependencies and system-provided Qt. Both amd64/arm64 passed install, offscreen/XCB self-tests and purge in the runtime container without a toolchain ([initial CI](https://github.com/Matte2599/WebFence/actions/runs/35876057107)). An [AT-SPI check](../evidence/debian-atspi-2026-09-24.md) and a [headless EN/IT Orca speech-command trial](../evidence/debian-orca-headless-2026-09-24.md) with null audio output were subsequently added. These trials are not a physical desktop, Wayland or heard-audio check. [ADR-007](ADR-007-PACKAGING.md) documents the procedure and limits.
...
Optional attachment is now implemented with `WEBFENCE_NATIVE_SOURCE_MATERIALS`: [procedure](M0-SOURCE-MATERIALS.md). Regenerates notices from reverified archives and includes them before signing; full archives remain separate. Local bundle with 247 notices, signature and Cocoa self-test verified. GLib/libb2 supplements are now also optionally attached before signing, bound to recipe hashes; distribution review remains open.

## Artifact-declared minimum system version

Packaging now reads `LC_BUILD_VERSION` or `LC_VERSION_MIN_MACOSX` from every included Mach-O after the ARM64 check. Records each file’s value in `minimum_macos` and the maximum in the inventory’s `macos_minimum` object. Distinguishes deployment target from SDK and linker versions; non-macOS platforms, missing/ambiguous values and invalid versions stop packaging.

`scripts/macho_minos.py` updates `Contents/Info.plist` with `LSMinimumSystemVersion` in numeric `n.n.n` form, preserving a higher existing declaration. Writes occur only in staging before signing, without changing binary load commands. A separate per-architecture declaration needs explicit review; the current builder produces ARM64 only.
