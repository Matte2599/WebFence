...
- **IT:** I [74 notices del primo ZIP](windows-notice-linkage-2026-09-24.md), ora [136 file di avviso/attribuzione](windows-attribution-sidecars-2026-09-24.md), sono collegati per pacchetto ai 22 proprietari delle 43 DLL e confrontati con gli archivi binari. Restano da esaminare completezza per codice incorporato, scelte per espressioni `OR`, voci `custom`, eccezioni e obblighi di distribuzione. La firma degli otto input PGP con `SKIP` non è stata verificata; gli hash dei payload da soli non autenticano la provenienza. Ricostruzione/sostituzione dei binari e revisione legale restano aperte. `distribution_ready=false` e `corresponding_sources_complete=false` non cambiano.
- **EN:** The [74 notices in the first ZIP](windows-notice-linkage-2026-09-24.md), now [136 notice/attribution files](windows-attribution-sidecars-2026-09-24.md), are linked by package to the 22 owners of the 43 DLLs and compared with binary archives. Embedded-code completeness, choices in `OR` expressions, `custom` entries, exceptions and distribution duties still need review. The eight PGP inputs marked `SKIP` have not had their signatures verified; payload hashes alone do not authenticate provenance. Binary rebuild/replacement and legal review remain open. `distribution_ready=false` and `corresponding_sources_complete=false` are unchanged.
